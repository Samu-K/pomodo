...
const handleEnableICal = async () => {
	const tokenSetting = draftSettings.value.find(
		(s) => s.key === "iCal sync token"
	);
	const enabledSetting = draftSettings.value.find(
		(s) => s.key === "iCal sync enabled"
	);
	const urlSetting = draftSettings.value.find((s) => s.key === "iCal sync URL");

	if (urlSetting && !urlSetting.value) {
		// Set default URL if empty
		urlSetting.value = "https://kasame.net/pomodo/ical-sync";
	}

	if (tokenSetting && !tokenSetting.value) {
		tokenSetting.value = crypto.randomUUID();
	}
...
// Moved copyToClipboard above

const icalFeedUrl = computed(() => {
	const url = draftSettings.value.find((s) => s.key === "iCal sync URL")?.value;
	const token = draftSettings.value.find(
		(s) => s.key === "iCal sync token"
	)?.value;
	if (!url || !token) return "";
    ...
    <div class="flex-1 overflow-y-auto px-6 py-6">
      ...
      <section class="mb-8">
        ...
        <div class="py-4 border-b border-light-border dark:border-dark-border">
          <div v-if="draftSettings.find(s => s.key === 'iCal sync enabled')?.value !== 'true'" class="flex flex-col gap-4">
            <div>
