...
import { useTimerStore, TimerMode } from '../../../stores/timer'
...
// Global Stubs
const globalStubs = {
    VProgressCircular: { name: 'VProgressCircular', template: '<div class="v-progress-circular"><slot></slot></div>', props: ['indeterminate', 'modelValue'] },
    CategoryManager: {
...
describe('TimerScreen.vue', () => {
    ...
    it('shows CategoryManager when timer is reset and not running', async () => {
        expect(wrapper.findComponent({ name: 'CategoryManager' }).exists()).toBe(true)
    })
