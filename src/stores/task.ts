...
export const useTasks = defineStore("tasks", () => {
    ...
    async function fetchTasks() {
        try {
            const fetched: any[] = await invoke("tasks_get_tasks");
            // Fetch categories to map names
            const categories: any[] = await invoke("categories_get_categories");
            ...
            tasks.value = fetched.map((t) => ({
                ...
                startTime: new Date(t.start_datetime),
    ...
    async function addTask(task: Task) {
        ...
        if (!categoryId && task.category) {
            try {
                ...
                const cats: any[] = await invoke("categories_get_categories");
                    ...
                    dates.forEach((date) => {
                        ...
                        if (!isException) {
                            expanded.push({
                                ...t,
                                startTime: date
                                // id: t.id is kept, but maybe we need a composite key for UI loops
                            } as any);
                        }
                    });
                } catch (e) {
